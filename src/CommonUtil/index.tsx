...
import { CommonColumnsType, ResponseEntity } from "cc-components/types";
...
const CommonUtil: React.FC<CommonUtilProps> = (props) => {
  ...
  return <Row className="m-2" justify="end" style={style}>
    <Space>
      {actions && !actions.find(action => action.key === 'import')?.hide &&
        <Upload
        ...
        >
          <Button
            ...
            icon={<CloudUploadOutlined style={{ fontSize: 20 }} />}>导入</Button>
        </Upload>
      }
      {actions && !actions.find(action => action.key === 'export')?.hide &&
        <Button
          key='export'
          type='primary'
          onClick={exportData}
