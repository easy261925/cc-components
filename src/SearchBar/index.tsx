import { Button, Card, Col, DatePicker, Form, Input, Select, Space } from "antd";
...
import { BaseEntity, CommonColumnsType, PaginationEntity, SearcherEntity } from "cc-components/types";
import '../index.less';
...
function SearchBar<T extends BaseEntity>(props: SearchBarProps<T>) {
  const { columns, getDataByPage, formInstance, onChange, style, colSpan = 6 } = props
  ...
  const renderSearchForm = (collapsed: boolean) => {
    ...
    return <Card bodyStyle={{ padding: '10px 24px' }} style={style}>
      <Form form={formInstance} layout='inline'>
        {allOptions.map(item => {
          const show = showOptions.find(option => option.dataIndex === item.dataIndex)
          ...
          switch (item.type) {
            case 'select':
              ...
              >
                {item?.options?.map(option => <Select.Option value={option.value}
            ...
            case 'date':
              ...
              formItem = <DatePicker
                onChange={val => {
                  onSearch({ [item.dataIndex]: moment(val).format('YYYY-MM-dd') })
                }}
                allowClear={false}
                className="w-full"
                placeholder={item.placeholder || `请选择${item.title}`}
            ...
            case 'dateMonth':
              ...
              formItem = <DatePicker.MonthPicker
                onChange={val => {
                  onSearch({ [item.dataIndex]: moment(val).format('YYYY-MM') })
                }}
                allowClear={false}
                className="w-full"
                placeholder={item.placeholder || `请选择${item.title}`}
            ...
            case 'dateYear':
              ...
              formItem = <DatePicker.YearPicker
                onChange={val => {
                  onSearch({ [item.dataIndex]: moment(val).format('YYYY') })
                }}
                allowClear={false}
                className="w-full"
                placeholder={item.placeholder || `请选择${item.title}`}
              ...
              break;
            default:
              formItem = <Input placeholder={item.placeholder || `请输入${item.title}`} allowClear />
              break;
          }
        ...
        <Col
          ...
          <Space>
            ...
            {needCollapseRender && <div onClick={() => setCollapsed(!collapsed)}>
