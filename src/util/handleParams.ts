import { CommonColumnsType, SearcherEntity } from "cc-components/types";
 ...
 * @returns 处理结果
 */
export function handleSearchParams<T>(searcher: SearcherEntity, columns?: CommonColumnsType<T>[]): any {
  ...
  for (const key in searcher) {
    if (Object.prototype.hasOwnProperty.call(searcher, key)) {
      ...
      if (moment.isMoment(element) && columns) {
        if (columns?.find(col => col.dataIndex === key)?.type === 'dateMonth') {
          newSearch[key] = moment(element).format('YYYY-MM')
        } else if (columns?.find(col => col.dataIndex === key)?.type === 'dateYear') {
...
export function handleParams<T>(params: T, columns?: CommonColumnsType<T>[]) {
  ...
  for (const key in params) {
      if (Object.prototype.hasOwnProperty.call(params, key)) {
          ...
          if (columns?.find(col =>
              col.dataIndex === key)?.type === 'dateMonth'
