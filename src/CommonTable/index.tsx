...
import { cloneElement, CSSProperties, useEffect, useState } from "react";
import { RedoOutlined } from '@ant-design/icons';
import { FormInstance } from "antd/es/form";
import SearchBar from "cc-components/SearchBar";
import CommonUtil from "cc-components/CommonUtil";
import { CommonColumnsType, FormModeEnum, PageResponseEntity, PaginationEntity, ResponseEntity } from "cc-components/types/CommonEntity";
import React from "react";
import { handleParams, handleSearchParams } from "cc-components/util/handleParams";
import { BaseEntity } from "cc-components/types";
...
function CommonTable<T extends BaseEntity>(props: CommonTableProps<T>) {
    ...
    const [tableColumns, setTableColumns] = useState<CommonColumnsType<T>[]>(columns?.filter(item => !item.hideInTable) || [])
    ...
    useEffect(() => {
        const optionColumn: CommonColumnsType<T> = {
            ...
            dataIndex: 'option',
            align: 'center',
            render: (text: string, entity: T) => <Space>
                <a onClick={() => {
                ...
                {updateService && <a onClick={() => {
                    setFormMode(FormModeEnum.UPDATE)
                    const params = handleParams(entity, columns)
                    formInstance?.setFieldsValue(params)
                    setOpen(true)
                }}>修改</a>}
                {deleteService && <Popconfirm
                    title='确认删除?'
                    onConfirm={() => {
        ...
        if (columns && columns.length > 0 && !columns.find(column => column.dataIndex === 'option')) {
            setTableColumns(columns.filter(item => !item.hideInTable).concat(optionColumn))
        }
    }, [columns]);
    ...
    const onOK = () => {
        formInstance?.validateFields().then(values => {
            ...
            if (formMode === FormModeEnum.CREATE) {
                ...
                setLoading(true)
                createService(newValues).then(res => {
                    if (res.success) {
                ...
                setLoading(true)
                updateService(newValues).then(res => {
                    if (res.success) {
    ...
    return <div className="flex-col">
        <SearchBar columns={columns} getDataByPage={getDataByPage} formInstance={searchFormRef} />
        <Row justify='end'>
            <Space>
                <CommonUtil
        ...
        <Modal
            ...
            footer={<Row justify='end'>
                {
                    formMode === FormModeEnum.VIEW ? <Button
